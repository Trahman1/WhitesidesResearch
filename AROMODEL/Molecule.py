...
class Molecule(object):
    ...
    def __init__(self, Name):
        ...
        self.COM -= self.COM
        print self.COM
        self.z_radius = - self.Atom_List[0].Position[1]
        print self.Atom_List[0].Element
        print "Radius = %.2f" % self.z_radius
        
        return
            

    def Adjust_COM(self, Random=True):
        if Random:
        ...
        if not Random:
            ...
            for Atom_Obj in self.Atom_List:
                # First rotation
                xt = Atom_Obj.Position[1]
                yt = Atom_Obj.Position[2]
                Atom_Obj.Position[1] = xt*C1 - yt*S1
                Atom_Obj.Position[2] = xt*S1 + yt*C1
                Atom_Obj.Position += self.COM
