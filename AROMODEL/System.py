...
class System(object):
    ...
    def Gen_Rand(self):
        ...
        for Molecule_Obj in self.Moltemp_List:
            ...
            if not Molecule_Obj.Molecule:
                Temp_Mol = deepcopy(Molecule_Obj)
                Temp_Mol.Mol_ID = k
                self.Molecule_List.append(Temp_Mol)
                print "Depositing surface"
                end
                k += 1
            
            if Molecule_Obj.Molecule:
                    ...
                    while not Deposited:
                        ...
                        try:
                            if len(self.Box_Size) > 1:
                                Temp_Mol.COM = np.asarray([30 +random.random()*(self.Box_Size[0] - 30), random.random()*(self.Box_Size[1] - 30), 100 + random.random()*(self.Box_Size[2]-400)], dtype = float)
                                Temp_Mol.Mol_ID = k
                        except:
                            Temp_Mol.COM = np.asarray([random.random()*self.Box_Size, random.random()*self.Box_Size, random.random()*self.Box_Size], dtype = float)
                            Temp_Mol.Mol_ID = k
        ...
        for Molecule_Obj in self.Molecule_List:
            print Molecule_Obj.Mol_ID, Molecule_Obj.Name
        return
...
"""
        if GPU:
            sub_temp = Configure.Template_Path + "P100_Sub"
            NProcs = 24
            submit = "sub_%s" % self.Name
            with open(sub_temp) as f:
                template = f.read()
                s = template.format(Sim_Name = Sim_Name, path = Configure.Comet_Path % self.Name, NProcs = Nodes*NProcs, Nodes=Nodes, tpn = NProcs, gpu_num = 2)
            with open(submit,'w') as f:
                f.write(s)
