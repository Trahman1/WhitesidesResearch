...
class DA_Polymer(object):
            ...
            if len(Line) == 1:
                ...
                if Line == ['Bonds']:
                    ...
                    for i in range(int(Num_Bonds)):
                        ...
                        self.Atom_List[int(Bond_Info[2])-1].Bond_List.append(int(Bond_Info[3])-1)
                        self.Atom_List[int(Bond_Info[3])-1].Bond_List.append(int(Bond_Info[2])-1)
    ...
    def Write_Mol2(self,Filename):
        # Scripts for writing out a .mol2 file given a DA Polymer object
        # HEADER
        file = open(Filename, 'w')
        file.write('@<TRIPOS>%s\n' % self.Name)
        file.write('*****\n')
        file.write('%d %d 0 0 0\n' %(len(self.Atom_List), len(self.Bond_List)))
        file.write('SMALL\n')
        ...
        for atom in self.Atom_List:
            file.write('      %d %s          %.4f    %.4f    %.4f %s.%d     1  LIG1       %.4f\n' % (atom.Atom_ID, atom.Element, atom.Position[0], atom.Position[1], atom.Position[2], atom.Element, len(atom.Bond_List), atom.Charge))
            i += 1
        # Bond Configuration
        i = 1
        file.write('@<TRIPOS>BOND\n')
        for bond in self.Bond_List:
            file.write('     %s     %s    %s    %s\n' % (i, bond.Bond_Master.Atom_ID, bond.Bond_Slave.Atom_ID, 1))
        return
